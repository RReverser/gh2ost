...
module.exports = function (options) {
	...
	if (options.diff) {
		var whenReadExisting = whenReadFile(options.filename, 'utf-8').then(
			function (contents) { return JSON.parse(contents).data.posts.map(function (post) { return post.slug.replace(/\W/g, '-') }) },
			function () { return [] }
		);
		...
		whenRepos = Promise.props({
			...
			return data.repos.filter(function (repo) {
				return data.existing.indexOf('gh-' + repo.name.toLowerCase().replace(/\W/g, '-')) < 0;
			});
		});
	}
